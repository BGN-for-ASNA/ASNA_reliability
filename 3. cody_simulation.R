...
# !! For each dyad, determine its sampling with and without bias according to previous steps.
  for ( i in 1:(N_id-1) ){
    for ( j in (i+1):N_id){
      ...
      ideal_samps[j,i] = # Network variables----------------------
#' @param N_id Number of individuals
#' @param B Tie probabilities
#' @param V Blocking variables (subgroups within the network)
#' @param groups Subgroup IDs
...
# !! Create an interaction matrix according to the ties probability matrix and observation bias.
  for ( i in 1:(N_id-1) ){
    for ( j in (i+1):N_id){
      y_true[i,j] = rbinom( 1 , size=true_samps[i,j], prob = # Network variables----------------------
#' @param N_id Number of individuals
#' @param B Tie probabilities
#' @param V Blocking variables (subgroups within the network)
#' @param groups Subgroup IDs
...
# !!  For each individual, determine:
 for( i in 1:N_id){
  ideal_exposure[i] = rpois(1, lambda=exposure_baseline) # !! Observation baseline.
  exposure_offset[i] = rnorm(1,0,exposure_sigma) # !! Observation variance.
  exposure_prob[i] = inv_logit(sum(exposure_effects*exposure_predictors[i,]) + exposure_offset[i]) # !! Its observation probabilities based on individual characteristics.
  true_exposure[i] = rbinom(1, size=ideal_exposure[i], prob=exposure_prob[i]) # !! Its observations with bias.
 }
...
int_mu = 10
int_sigma = 2
int_exposure_predictors =
if(int_bias){
  int_prob = rep(NA, N_id) #!! The probability of sample based on individual characteristics.
  int_exposure = rep(NA, N_id) #!! Observation time without bias.
  true_int = rep(NA, N_id) #!! Observation time with bias.
  ideal_int_exposure = rep(NA, N_id)
  int_exposure_offset = rep(NA, N_id) #!! Observation



  # !!  For each individual, determine:
  for( i in 1:N_id){
    ideal_int_exposure[i] = rpois(1, lambda=int_mu) # !! Interaction baseline.
    exposure_offset[i] = rnorm(1,0,int_sigma) # !! Observation variance.
    int_prob[i] = inv_logit(sum(exposure_effects*exposure_predictors[i,]) + exposure_offset[i]) # !! Its observation probabilities based on individual characteristics.
    true_exposure[i] = rbinom(1, size=ideal_exposure[i], prob=exposure_prob[i]) # !! Its observations with bias.
  }
