...
#'
## 2.2. There is a relationship between individual characteristics and sociality, but there is no relationship between individual characteristics, observation bias, and censoring
...
#'
## 2.3. There is no relationship between individual characteristics,  sociality and censoring, but there is a relationship between individual characteristics and exposure
...
## 2.4. There is no relationship between individual characteristics, sociality and exposure but there is a relationship between individual characteristics and censoring
...
for (a in a:length(TEST)) {
  for(b in 1:10){
    r[[length(r)+1]] = test.function(att = Hairy,
                           ...
                           sr_mu =  c(0, 0), sr_sigma =  c(0.01,0.01), # no sender-receiver effect
                           dr_mu = c(0,0), dr_sigma = 0.01, # no dyadic effect
                           exposure_predictors = NULL,
                           exposure_effects = c(0, 0), exposure_sigma = 0.01, # exposure effect
                           int_intercept = c(Inf,Inf), int_slope = c(Inf,Inf),#no censoring effect
...
# 3. Testing when the coefficient of exposure (exposure_effects parameter) lead to significant effect on simulated data -------
N_id = 30
Hairy = matrix(rnorm(N_id, 0, 1), nrow=N_id, ncol=1)
...
## 3.1. An example of exposure_effects being equal to 0.2 in simulated data
N_id = 50
Hairy = matrix(rnorm(N_id, 0, 1), nrow=N_id, ncol=1)
test = test.function(att = Hairy,
                     N_id = N_id,
...
# 4. Testing when the coefficient of censoring (int_slope parameter) lead to significant effect on simulated data -------
N_id = 30
Hairy = matrix(rnorm(N_id, 0, 1), nrow=N_id, ncol=1)
...
## 4.1. An example of censoring intercept and slope are equal to 0.1 in simulated data
N_id = 50
Hairy = matrix(rnorm(N_id, 0, 1), nrow=N_id, ncol=1)
test = test.function(att = Hairy,
                     N_id = N_id,
