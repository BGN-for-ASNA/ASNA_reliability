...
fit_social_relations_model_bias <- function (data, block_regression, focal_regression, target_regression, 
            dyad_regression, censoring_regression, mode = "mcmc", return_predicted_network = FALSE, 
            stan_mcmc_parameters = list(seed = 1, chains = 1, parallel_chains = 1, 
                                        refresh = 1, iter_warmup = NULL, iter_sampling = NULL, 
                                        max_treedepth = NULL, adapt_delta = NULL), priors = NULL) {
  ...
  # censoring----------------
  if (data$N_individual_predictors > 0) {
    data$censoring_set = model.matrix(censoring_regression, data$individual_predictors)
  }else {
  ...
  for (i in 1:data$N_group_vars) {
    data$N_groups_per_var[i] = length(unique(data$block_set[, 
                                                            i]))
  }
