...
model{
  ...
  for(i in 1:(N_id-1)){
    ...
    //for(i in 1:(N_id-1)){
      //for(j in (i+1):N_id){
        // int_intercept[1] = dr_raw[i,j];
        // int_intercept[2] = dr_raw[j,i];
        // scrap = rep_vector(dr_sigma, 2) .* (dr_L*scrap);
        // intp[i,j] = int_intercept[1] + dot_product(int_slope,  individual_predictors[i]);
        // intp[j,i] = int_intercept[2] + dot_product(int_slope,  individual_predictors[j]);
        // }}
        ...
        for(i in 1:N_id){
          for(j in 1:N_id){
            if(i != j){
              ...
              if(outcome_mode == 4){
                X_ij ~ binomial_logit(exposure[i,j,1], sum(br) + sr[i,1] + sr[j,2] + dr[i,j]);
                outcomes[i,j,1] ~ binomial_logit(Tehta[i,j,1], X_ij)
              }
            }
          }
        }
